...
export default {
  ...
  methods: {
    ...
    /* ===== Additions to satisfy runtime usage (was missing earlier) ===== */

    // Detects if a slot object is actually a fireteam header row.
    isFireteamHeader(slot, idx, slots) {
      try {
        if (!slot) return false;
        // explicit marker
        if (slot.isFireteamHeader === true || slot.header === true || slot.type === "header") return true;

        const candidate = String(slot.name || slot.role || slot.label || "").trim();
        if (!candidate) return false;

        const low = candidate.toLowerCase();
        // common header patterns
        if (low.startsWith("fireteam") || low.startsWith("element") || low.startsWith("ft ")) return true;

        // Some ORBATs store header as a slot with no id and a role like "Fireteam 1"
        if (!slot.id && /fireteam|element/i.test(String(slot.role || slot.name || ""))) return true;
    ...
    // Human label for a fireteam header slot
    fireteamLabel(slot) {
      if (!slot) return "Element";
      const n = slot.name || slot.label || slot.role || slot.title || "";
      if (!n) return "Element";
      // Some values may contain "Fireteam 1" or similar — normalize spacing
    ...
    /* the remainder of the helper implementations were included earlier in this file */
  }
};
</script>
